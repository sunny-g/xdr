...
defmodule XDR.Type.Uint do
  ...
  def encode(int), do: {:ok, <<int :: unsigned-size(32)>>}

  def decode(xdr) when not is_binary(xdr), do: {:error, :invalid}
  def decode(xdr) when bit_size(xdr) > 32, do: {:error, :out_of_bounds}
  def decode(<<int :: unsigned-size(32)>>) when not is_integer(int), do: {:error, :invalid}
  def decode(<<int :: unsigned-size(32)>>), do: {:ok, int}
